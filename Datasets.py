...
class MyDataset(Dataset):
    def __init__(self, num=0, file='', choice='train',input_size=512,gt_ratio=1):
        ...
        self.albu = A.Compose([
            ...
            A.ImageCompression(quality=(50, 95), p=0.75),
            A.OneOf([
                A.OneOf([
                    A.Blur(p=1),
                    A.GaussianBlur(p=1),
    ...
    def __len__(self):
        return self.num
    def load_item(self, idx, retry=0):
        fname1, fname2 = self.filelist[idx]
        img = cv2.imread(fname1)
        if img is None:
            print(f"[WARNING] Failed to load image: {fname1}")
            if retry >= 10:
                raise RuntimeError(f"[FATAL] Too many retries at idx {idx}")
            return self.load_item((idx + 1) % len(self), retry=retry+1)
        img = img[..., ::-1]
        H, W, _ = img.shape
        mask = cv2.imread(fname2) if fname2 != '' else None
        if mask is None or not isinstance(mask, np.ndarray):
            print(f"[WARNING] Failed to load mask: {fname2}, using zero mask.")
