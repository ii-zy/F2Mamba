...
def get_device(cuda_idx):
    ...
    return device
with open('/home/law/HDD/i_zzy/00_NEMLoc/config_NEMLoc.yaml', 'r') as f:
...
class SS2Dv0:
    def __initv0__(
            self,d_model=96,d_state=16,ssm_ratio=2.0,dt_rank="auto",dropout=0.0,seq=False,force_fp32=True,
            **kwargs,
    ):
...
class SS2Dv2:
    def __initv2__(
            self,d_model=96,d_state=16,ssm_ratio=2.0,dt_rank="auto",act_layer=nn.SiLU,d_conv=3, conv_bias=True,dropout=0.0,bias=False,dt_min=0.001,dt_max=0.1,dt_init="random",dt_scale=1.0,dt_init_floor=1e-4,initialize="v0",forward_type="v2",channel_first=False,
            **kwargs,
    ):
        factory_kwargs = {"device": None, "dtype": None}
        super().__init__()
...
class SS2Dv3:
    def __initxv__(
            self,d_model=96,d_state=16,ssm_ratio=2.0,dt_rank="auto",d_conv=3, conv_bias=True,dropout=0.0,bias=False,dt_min=0.001,dt_max=0.1,dt_init="random",dt_scale=1.0,dt_init_floor=1e-4,initialize="v0",forward_type="v2",channel_first=False,
            **kwargs,
    ):
        super().__init__()
        d_inner = int(ssm_ratio * d_model)
...
class SS2Dm0:
    def __initm0__(
            self,d_model=96,d_state=16,ssm_ratio=2.0,dt_rank="auto",act_layer=nn.GELU,d_conv=3,conv_bias=True,dropout=0.0,bias=False,dt_min=0.001,dt_max=0.1,dt_init="random",dt_scale=1.0,dt_init_floor=1e-4,initialize="v2",forward_type="m0",with_initial_state=False,
            **kwargs,
    ):
        factory_kwargs = {"device": None, "dtype": None}
        super().__init__()
...
class LowFreqEnhancer(nn.Module):
    def __init__(self, dim):
        super().__init__()
        self.low_mod_conv = nn.Sequential(
    ...
    def forward(self, x, low_feat):
        attn = self.low_mod_conv(low_feat)
        return x * attn
...
class SS2D(nn.Module, SS2Dv0, SS2Dv2, SS2Dv3, SS2Dm0):
    def __init__(
        ...
        super().__init__()
        self.low_freq_enhancer = LowFreqEnhancer(d_model)
    ...
    def forward(self, x: torch.Tensor, low_feat: Optional[torch.Tensor] = None):
        shortcut = x
        x = self.patch_embed(x)
        x = self.norm1(x)
        x = self.activation(x)
        x = self.global_response_norm(x)
        x = self.ssm(x)
        x = self.norm2(x)
        x = self.conv_proj(x)
        ...
        if low_feat is not None:
            x = self.low_enhancer(x, low_feat)
